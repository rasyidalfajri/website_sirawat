...
$sql = "SELECT s.*, a.nama_aset FROM maintenance_schedule s JOIN assets a ON a.id=s.asset_id WHERE $where ORDER BY s.tanggal DESC";
...
?>
<h4 class="mb-3">Laporan Pemeliharaan</h4>
    ...
    <div class="col-md-4">
      ...
      <select name="asset_id" class="form-select">
        ...
        <?php while($a = $assets->fetch_assoc()): ?>
          <option value="<?= $a['id'] ?>" <?= (($_GET['asset_id']??'')==$a['id'])?'selected':'' ?>><?= htmlspecialchars($a['nama_aset']) ?></option>
        <?php endwhile; ?>
      </select>
    </div>
    <div class="col-md-3">
...
<div class="card shadow-sm border-0">
  ...
  <div class="table-responsive">
    <table class="table table-striped mb-0">
      <thead>
        <tr><th>Tanggal</th><th>Aset</th><th>Jenis</th><th>Deskripsi</th><th>Status</th></tr>
      </thead>
      <tbody>
        <?php while($r = $list->fetch_assoc()): ?>
          <tr>
            ...
            <td><?= htmlspecialchars($r['jenis_perawatan']) ?></td>
